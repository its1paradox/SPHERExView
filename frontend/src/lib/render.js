...
// ---------------------------------------------------------------------------
// DIFFUSE-EMISSION ENHANCEMENT ("ring finder")
//
// WiseView reveals faint extended structure (nebular rings, shells) with a
// LINEAR stretch clamped just above the sky level (narrow min/max window).
// In crowded low-latitude fields the stars still bury arcmin-scale rings,
// so before stretching we suppress point sources with a star-clip:
//
//   1. local median (win x win window) = star-free background estimate;
//   2. ONLY pixels brighter than med + 2.5 robust-sigma are replaced by
//      that local median -- faint extended emission passes untouched
//      (a plain median filter would also erase the thin rim of a ring);
//   3. light gaussian smooth (sigma ~1.2 px) knits large-scale structure;
//   4. narrow linear window: med - 0.5 sigma .. med + 3.5 sigma.
//
// Robust sigma = 1.4826 * MAD (median absolute deviation), immune to the
// very stars being clipped. Validated on the elliptical ring at
// RA 66.7727 Dec +34.2532 (visible in W2 / SPHEREx D5).

function medianOfWindow(vals, n) {
  vals.length = n;
  vals.sort((a, b) => a - b);
  return n % 2 ? vals[(n - 1) >> 1] : 0.5 * (vals[n / 2 - 1] + vals[n / 2]);
}

function localMedianFilter(data, w, h, win) {
  const half = win >> 1;
  ...
  for (let y = 0; y < h; y += 1) {
    for (let x = 0; x < w; x += 1) {
      ...
      out[y * w + x] = n ? medianOfWindow(buf, n) : 0;
    }
  }
  return out;
}

function gaussianSmooth(data, w, h, sigma) {
...
// Per-array cache: the enhancement is deterministic, frames are immutable.
const diffuseCache = new WeakMap();

// Star-clip + smooth one float array. Returns a NEW Float32Array.
export function diffuseEnhanceArray(data, w, h) {
  const hit = diffuseCache.get(data);
  if (hit) return hit;
  // Robust stats over finite pixels.
  const finite = [];
  for (let i = 0; i < data.length; i += 1) if (Number.isFinite(data[i])) finite.push(data[i]);
  finite.sort((a, b) => a - b);
  const med = finite.length ? finite[finite.length >> 1] : 0;
  const dev = finite.map((v) => Math.abs(v - med)).sort((a, b) => a - b);
  const sigma = 1.4826 * (dev.length ? dev[dev.length >> 1] : 1) || 1;
  // NaN -> sky level so blank corners do not bleed into the filters.
  const filled = new Float32Array(data.length);
  for (let i = 0; i < data.length; i += 1) filled[i] = Number.isFinite(data[i]) ? data[i] : med;
  // Window: > PSF footprint. SPHEREx grids (~6.2"/px) are small, unWISE
  // grids (2.75"/px) larger -- scale the window with the array size.
  const win = Math.max(w, h) >= 150 ? 7 : 5;
  const loc = localMedianFilter(filled, w, h, win);
  const thresh = med + 2.5 * sigma;
  const clipped = new Float32Array(data.length);
  for (let i = 0; i < data.length; i += 1) clipped[i] = filled[i] > thresh ? loc[i] : filled[i];
  const out = gaussianSmooth(clipped, w, h, 1.2);
  diffuseCache.set(data, out);
  return out;
}
...
// recomputes the sorted pixel lists used by the display-limit machinery.
export function diffuseFrame(f) {
  const data = diffuseEnhanceArray(f.data, f.width, f.height);
  const out = { ...f, data, sorted: sortPixels(data) };
  if (f.data2) {
    out.data2 = diffuseEnhanceArray(f.data2, f.width, f.height);
    out.sorted2 = sortPixels(out.data2);
  }
  return out;
}
