...
const BAND_OPTIONS = [
  ...
  { value: 'custom', label: 'Custom channels (COLOR)' },
...
function parseBlinkHash() {
  ...
  return {
    ...
    ref: p.get('ref') || 'auto',
    // Coadd recipe — defaults reproduce the historical blink behavior
    // exactly (zodi subtraction, 5-sigma x2 clipping, native 6.2" nearest).
    background: p.get('background') || 'zodi',
    sigma: p.get('sigma') || '5',
    maxiters: p.get('maxiters') || '2',
    minexp: p.get('minexp') || '1',
    pixscale: p.get('pixscale') || '6.2',
    resampling: p.get('resampling') || 'nearest',
    short: p.get('short') || '1,2,3,4',
    long: p.get('long') || '5,6',
  };
}
...
export default function BlinkPage() {
  ...
  const build = async (f) => {
    ...
    const params = new URLSearchParams({
      ...
      sigma: f.sigma,
      maxiters: f.maxiters,
      min_channel_exposures: f.minexp,
      pixscale_arcsec: f.pixscale,
      resampling: f.resampling,
    });
    if (f.band === 'custom') {
      const shortDets = parseDets(f.short);
      const longDets = parseDets(f.long);
      if (!shortDets.length || !longDets.length) {
        setError('Custom channels need at least one detector on each side.');
        setLoading(false);
        return;
      }
      params.set('short_detectors', shortDets.join(','));
      params.set('long_detectors', longDets.join(','));
    } else if (f.band && f.band !== 'all') {
    ...
    try {
      ...
      window.location.hash = new URLSearchParams({
        ...
        ...(f.band === 'custom'
          ? { short: parseDets(f.short).join(','), long: parseDets(f.long).join(',') }
          : f.band && f.band !== 'all'
            ? { band: f.band, ref: f.ref || 'auto' }
            : {}),
        ...(f.background !== 'zodi' ? { background: f.background } : {}),
        ...(String(f.sigma) !== '5' ? { sigma: f.sigma } : {}),
        ...(String(f.maxiters) !== '2' ? { maxiters: f.maxiters } : {}),
        ...(String(f.minexp) !== '1' ? { minexp: f.minexp } : {}),
        ...(String(f.pixscale) !== '6.2' ? { pixscale: f.pixscale } : {}),
        ...(f.resampling !== 'nearest' ? { resampling: f.resampling } : {}),
      }).toString();
    } catch (err) {
      setStatus(null);
  ...
  const toggleDet = (channel, d) =>
    setForm((fm) => {
      const cur = parseDets(fm[channel]);
      const next = cur.includes(d)
        ? cur.filter((x) => x !== d)
        : [...cur, d].sort((a, b) => a - b);
      return { ...fm, [channel]: next.join(',') };
    });
    ...
    <div className="app blink-app">
      <header>
        ...
        <p className="subtitle">
          {form.band === 'custom'
            ? `Custom two-channel color blink \u2014 one coadd per sky-pass visit, your own detector groups feeding the blue and orange channels in the WiseView color language (short \u2192 blue \u00b7 long \u2192 orange \u00b7 equal \u2192 white)`
      ...
      <div className="layout">
        ...
        >
          <fieldset>
            ...
            {form.band !== 'all' && form.band !== 'custom' && (
              <label>
                Reference channel
            ...
            {form.band === 'custom' && (
              <>
                ...
                <div className="detector-grid">
                  {[1, 2, 3, 4, 5, 6].map((d) => (
                    <label className="check" key={`short-${d}`}>
                      <input
                        type="checkbox"
                        checked={parseDets(form.short).includes(d)}
                        disabled={parseDets(form.long).includes(d)}
                        onChange={() => toggleDet('short', d)}
                      />
                      D{d}
                    </label>
                  ))}
                ...
                <div className="detector-grid">
                  {[1, 2, 3, 4, 5, 6].map((d) => (
                    <label className="check" key={`long-${d}`}>
                      <input
                        type="checkbox"
                        checked={parseDets(form.long).includes(d)}
                        disabled={parseDets(form.short).includes(d)}
                        onChange={() => toggleDet('long', d)}
                      />
                      D{d}
                    </label>
                  ))}
            ...
            <p className="hint">
              {form.band === 'custom'
                ? `Your detector groups define the two color channels: everything in the top group feeds blue, everything in the bottom group feeds orange. Wavelength order is preserved by convention \u2014 keep shorter detectors in the blue group so hues stay physically meaningful.`
                : form.band !== 'all'
          ...
          <fieldset>
            ...
            <details className="coadd-settings">
              <summary>Epoch coadd recipe</summary>
              <label>
                Background treatment
                <select value={form.background} onChange={setF('background')}>
              ...
              <label>
                ...
                <input
                  ...
                  max="100"
                  value={form.minexp}
                  onChange={setF('minexp')}
                />
              </label>
              <label>
                Output pixel scale
                <select value={form.pixscale} onChange={setF('pixscale')}>
              ...
              <label>
                Resampling
                <select value={form.resampling} onChange={setF('resampling')}>
                  <option value="nearest">Nearest (pixel preserving)</option>
                  <option value="bilinear">Bilinear (clearer display)</option>
                </select>
              </label>
              <p className="hint">
                {`3.1\u2033 bilinear output reduces blockiness and can use sub-pixel visit offsets, but it does not create angular resolution beyond SPHEREx. The default 6.2\u2033 nearest is the most conservative pixel-preserving product. Recipe changes apply on the next build.`}
              </p>
            </details>
          </fieldset>
