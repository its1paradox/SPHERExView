...
export default function CombinedViewer({
  ...
  useEffect(() => {
    ...
    // v19 renderer: grayscale epochs get their black/white points through
    // vmin/vmax (percentileLimits above); Lupton color frames keep the
    // frozen sequence-wide scale, so no live percentiles are passed here.
  ...
  if (frames.length === 0) return (
    <section className="panel viewer combined-viewer">
      ...
      <p className="timeline-placeholder">No dated frames with usable sky coordinates cover this target.</p>
    </section>
  );
  ...
  return (
    <section className="panel viewer combined-viewer">
      ...
      {pin && (
        <div className="timeline-pin" aria-label="Pinned source">
          <span>{pin.ra.toFixed(7)}°, {pin.dec.toFixed(7)}° ICRS</span>
          <button type="button" onClick={async () => {
            try {
              await navigator.clipboard.writeText(`${pin.ra.toFixed(7)} ${pin.dec.toFixed(7)}`);
              setCopyStatus('Coordinates copied');
            } catch {
              setCopyStatus('Select the coordinates to copy them');
            }
          }}>Copy coordinates</button>
          {onSpectrum && <button type="button" onClick={() => onSpectrum(pin.ra, pin.dec)}>Generate spectrum at pin</button>}
          <button type="button" onClick={() => onPin?.(null)}>Clear pin</button>
