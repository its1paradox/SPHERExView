...
export default function DetectorComparison() {
  ...
  const exportFigure = () => {
    ...
    ctx.font = '18px sans-serif'; ctx.fillText(`${result.recipe.ra.toFixed(6)}°, ${result.recipe.dec.toFixed(6)}° ICRS  |  ${currentEpoch.data_release?.toUpperCase()} | ${currentEpoch.datetime_start} — ${currentEpoch.datetime_end} UTC`, 28, 78);
  ...
  return <div className="compare-page" data-mode={display.mode}>
    <header className="compare-header"><div><a href="index.html" className="compare-brand">SpherexMultiView</a><h1>Six-detector comparison</h1></div><span className="comparison-tag">QR2 + QR3 · ICRS</span></header>
    <div className="compare-layout">
      <main className="compare-main">
        ...
        {currentEpoch && <div className="epoch-caption"><strong>{currentEpoch.datetime_start.slice(0, 10)} — {currentEpoch.datetime_end.slice(0, 10)}</strong><span>{currentEpoch.data_release?.toUpperCase()} · {currentEpoch.grouping} · UTC · actual detector times below each tile</span></div>}
      ...
      <aside className="compare-sidebar">
        ...
        <form onSubmit={build}><fieldset disabled={busy}><legend>Target &amp; coadd recipe</legend>
          ...
          <ReleaseSelect value={form.release} onChange={e => update('release', e.target.value)} />
        ...
        {result && <fieldset><legend>Export &amp; provenance</legend><button onClick={exportFigure}>Export six-panel PNG + settings</button><a className="compare-download" href={`/api/detector-comparison/${resultJobId}/epochs/${currentEpoch.index}.fits`}>Download epoch FITS</a><a className="compare-download" href={`/api/detector-comparison/${resultJobId}/manifest`}>Download full manifest</a><p>FITS includes intensity, variance, coverage, weights, wavelength/time maps, WCS and input provenance. QR2 and QR3 use their native calibrations and remain in separate coadds. PNG retains labels and display limits.</p><details><summary>D{selected} input provenance ({currentInputs.length})</summary><div className="comparison-inputs">{currentInputs.length ? currentInputs.map((input, i) => <article key={i}><a href={input.url} target="_blank" rel="noreferrer">{input.obs_id}</a><span>{input.status.replaceAll('_', ' ')}</span>{input.reason && <p>{input.reason}</p>}{input.pipeline_version && <small>{input.data_release?.toUpperCase()} · Pipeline {input.pipeline_version} · PSF header: {input.psf_header_status.replaceAll('_', ' ')}</small>}</article>) : <p>No observations for D{selected} in this epoch.</p>}</div></details></fieldset>}
